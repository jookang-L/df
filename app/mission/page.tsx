...
import { submitMissionAttempt } from "@/app/actions/mission";
...
export default function MissionPage() {
  ...
  const handleRun = (code: string) => {
    ...
    if (!result || !currentMission) return;
    ...
    setTimeout(async () => {
      if (result.finalData.length > 0) {
        ...
        if (isCorrect) {
          const wrongAttempts = wrongAttemptsByMission[currentMission.id] ?? 0;
          const dbResult = await submitMissionAttempt({
          ...
          if (dbResult.success) {
            if (dbResult.alreadySolved) {
              setEarnedScoreMsg("이미 풀었던 문제입니다. 점수는 추가되지 않습니다.");
            } else if (dbResult.earnedScore !== undefined) {
              setEarnedScoreMsg(`+${dbResult.earnedScore}점 획득! (총 ${dbResult.totalScore ?? 0}점)`);
            }
          }
          ...
          setWrongAttemptsByMission((prev) => ({
            ...prev,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {student && (
          <motion.div
            ...
            className="mb-4 bg-green-700 text-white rounded-xl px-4 py-3 text-sm font-medium"
          >
            안녕하세요, <span className="font-bold text-yellow-300">{student.name}</span>님! 오늘의 미션에 도전해보세요.
          </motion.div>
        )}
        ...
        <AnimatePresence>
          {showSuccess && (
            ...
            >
              ...
              {earnedScoreMsg && (
                <div className="text-sm font-semibold mt-2 bg-white/20 rounded-lg px-3 py-1.5">
                  {earnedScoreMsg}
                </div>
              )}
